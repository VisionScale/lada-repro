...
echo "Checking Office-31 data in ${HOST_DATA_DIR}/office ..."
if [[ ! -d "${HOST_DATA_DIR}/office/amazon" || ! -d "${HOST_DATA_DIR}/office/webcam" || ! -d "${HOST_DATA_DIR}/office/dslr" ]]; then
    ...
    echo "  ${HOST_DATA_DIR}/office/amazon"
    echo "  ${HOST_DATA_DIR}/office/webcam"
    echo "  ${HOST_DATA_DIR}/office/dslr"
    exit 1
fi
...
docker run --rm --gpus "device=${GPU_ID}" \
    -v "${HOST_DATA_DIR}/office:${CONTAINER_DATA_DIR}/office" \
